...
public class EcContainerPortableEnchantment extends Container
{
            ...
            if (var2 != null && var2.isItemEnchantable())
            {
                ...
                if (!this.worldPointer.isRemote)
                {
					...
					*/
                    for(int i=0;i<36;i++)
                    {
                    	if(InvP.getStackInSlot(i) != null && InvP.getStackInSlot(i).itemID ==Block.bookShelf.blockID)
                    	{
    ...
    {
        return true;
    	//return this.worldPointer.getBlockId(this.posX, this.posY, this.posZ) != mod_EnchantmentTable2.EnchantmentTable2ID ? false : par1EntityPlayer.getDistanceSq((double)this.posX + 0.5D, (double)this.posY + 0.5D, (double)this.posZ + 0.5D) <= 64.0D;
    }
