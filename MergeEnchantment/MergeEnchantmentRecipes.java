...
public class MergeEnchantmentRecipes implements IRecipe
{
	...
	public boolean matches(InventoryCrafting inv, World world)
	{
		...
		for(int i=0; i< inv.getSizeInventory();i++)
		{
			...
			if(craftitem !=null)
			{
				if(craftitem.getItem().isRepairable() && craftitem.itemID != Item.book.itemID)
				{
					if(items[0] == null)
					{
						toolflag++;
					...
					else if(items[0].itemID == craftitem.itemID && toolflag ==1)
					{
				...
				else if(craftitem.itemID == Item.book.itemID && bookflag == 0)
				{
					bookflag++;
					continue;
				}
	...
	public ItemStack getCraftingResult(InventoryCrafting inventorycrafting)
	{
		...
		output = new ItemStack(items[0].itemID, 1, items[0].getItemDamage());
		SameEnchindex =0;
		for (int i = 0; i < alist.size(); i++) {
